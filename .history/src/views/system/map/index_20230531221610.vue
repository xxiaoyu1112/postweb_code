...
methods: {
    ...
    setMark(marks){ 
        ...
        // 设置鼠标划过点标记显示的文字提示
        this.marker.setTitle('快递包裹title');
        // // 设置label标签
    ...
    markerClick(e){
        ...
        var content ='<strong style="color:orange">当前快递</strong><br/>'
            + '<ul> 经纬度'+data.latlng+'</ul>'
            + '<ul> 地址：'+data.address+'</ul>';
        // 设置信息框内容
        // this.marker.setContent(content);
        //将marker放到指定坐标
        // this.marker.open(this.map, data.position);
        new AMap.InfoWindow({
            content:content,
            offset: new AMap.Pixel(0, -30)
        }).open(this.map, data.positionvv);
    },
    setMapCenter(center){
        // console.log(center);
