...
watch: {
    "amapContext.data.markers" (newMarks,oldMarks){
        ...
        this.amapContext.data.markers.forEach((marker) => {
            this.setMark(marker.position)
        });
        this.setMapCenter(this.amapContext.data.markers[0].position)
        // console.log(this.amapContext.data.markers);
...
methods: {
    initAMap() {
        AMapLoader.load({
            ...
            version: "2.0",
            plugins: ["AMap.ToolBar", "AMap.Driving","AMap.Riding", 'AMap.AutoComplete','AMap.Walking'],
            AMapUI: {
                version: "1.1",
                plugins: [],
            },
    ...
    ridingRoute(){
        ...
        //     panel: "panel"
        // });
        // console.log(riding.opt.panel);
        //根据起终点坐标规划骑行路线
        riding.search([120.157059,30.29697],[120.157059,30.29697], function(status, result) {
            console.log(status, result);
            // result即是对应的骑行路线数据信息，相关数据结构文档请参考  https://lbs.amap.com/api/javascript-api/reference/route-search#m_RidingResult
            if (result.routes && result.routes.length) {
                this.setMark([120.157059,30.29697])
    ...
    //   console.log(marker.location.split(","))
    //   console.log(this.amapContext.map.center)
      let center = marker.location.split(",")
      this.setMapCenter([parseFloat(center[0]),parseFloat(center[1])])
    },
},
};
</script>
